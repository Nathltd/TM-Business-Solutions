...
VALUES (<cfqueryparam cfsqltype="cf_sql_clob" maxlength="8" value="#session.SalesID#">,<cfqueryparam value="#dateformat(session.myDate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#session.BranchID#',  'General', '#session.StaffID#', <cfqueryparam value="#amountPaid#" cfsqltype="cf_sql_double">, <cfqueryparam value="#dateformat(session.todate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#GetAuthUser()#', '#session.AccountID#','Order')
</cfquery>
<cfquery>
...
<div align="right"> <a href="#cgi.SCRIPT_NAME#"> New Invoice </a>  |  <a href="../invoices/invoice.cfm?cfgridkey=#url.cfgridkey#&InvoiceType=Original">Print</a>&nbsp;</div>
    ...
    <tr>
      ...
      <td><div align="left">
       ...
       where userid = '#GetAuthUser()#'
        ...
        <cfelse>
        <cfselect name="StaffID" id="StaffID" tooltip="Staff that signed the Sales Order" required="yes" message="Select Authorised" bind="cfc:#request.cfc#cfc.bind.getStaff({BranchID})" display="FirstName" value="FirstName" bindonload="yes" tabindex="6" style="text-align:left; width:100%">
        </cfselect>
        </cfif>
       </cfif>
      </div></td>
