...
<html>
<head>
<title><cfoutput>Sales Order :: #request.title#</cfoutput></title>
...
<cfquery>
INSERT INTO Sales
(SalesID,SalesDate,BranchId,CustomerId,StaffID,AmountPaid,LastUpdated,Creator,AccountId,Type)
VALUES (<cfqueryparam cfsqltype="cf_sql_clob" maxlength="8" value="#session.SalesID#">,<cfqueryparam value="#dateformat(session.myDate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#session.BranchID#',  '#session.customerID#', '#session.StaffID#', <cfqueryparam value="#amountPaid#" cfsqltype="cf_sql_double">, <cfqueryparam value="#dateformat(session.todate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#GetAuthUser()#', '#session.AccountID#','Order')
</cfquery>
...
<div align="right"> <a href="#cgi.SCRIPT_NAME#"> New Sales Order </a>  |  <a href="../invoices/invoice.cfm?cfgridkey=#url.cfgridkey#&InvoiceType=ORIGINAL">Print</a>&nbsp;</div>
 Order No. <strong>#session.SalesID#</strong> have been posted successfully
 </cfoutput><br>
<br>
<table>
<cfoutput> 
...
<td></td>
<td></td>
<td><div align="right">Order No:</div></td>
    ...
    <tr>
      ...
      <td><div align="left">
       ...
       where userid = '#GetAuthUser()#'
        ...
        <cfelse>
        <cfselect name="StaffID" id="StaffID" tooltip="Staff that signed the Order" required="yes" message="Select Authorised" bind="cfc:#request.cfc#cfc.bind.getStaff({BranchID})" display="FirstName" value="FirstName" bindonload="yes" tabindex="6" style="text-align:left; width:100%">
        </cfselect>
        </cfif>
       </cfif>
      </div></td>
