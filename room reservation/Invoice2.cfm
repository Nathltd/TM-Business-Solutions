...
VALUES (<cfqueryparam cfsqltype="cf_sql_clob" maxlength="8" value="#session.SalesID#">,<cfqueryparam value="#dateformat(session.myDate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#session.BranchID#',  '#session.customerID#', '#session.StaffID#', <cfqueryparam value="#amountPaid#" cfsqltype="cf_sql_double">, <cfqueryparam value="#dateformat(session.todate,"dd/mm/yyyy")#" cfsqltype="cf_sql_timestamp">, '#GetAuthUser()#', '#session.AccountID#')
</cfquery>
...
<div>
  ...
  <table width="90%" cellpadding="2">
        ...
        <cfif isdefined('AddInvoice')>
          ...
          <cfselect name="CustomerID"  id="CustomerID" title="Customer's Name" message="Select a Customer"  tabindex="1" autofocus style="text-align:left; width:100%">
            <cfloop query="Customers">
              <option value="#Trim(CustomerID)#">#CustomerID#</option>
              </cfloop>
            </cfselect>
        </cfif>
      </div></td>
    ...
    <tr>
      ...
      <td><div align="left">
        <cfif isdefined('AddInvoice')>
          ...
          <cfelse>
          <cfinput name="SalesID" type="text" id="SalesID" validate="integer" required="yes" message="Enter an Invoice number" tooltip="Invoice Number" tabindex="3" autocomplete="off" style="text-align:right; width:100%" bind="cfc:#request.cfc#cfc.bind.invNumber()" bindonload="yes">
      ...
      <td width="168"><div align="left">
       ...
       where userid = '#GetAuthUser()#'
        ...
        <cfelse>
            <cfselect name="BranchID" id="BranchID" required="yes"  title="Select Sales Location" tabindex="4" style="text-align:left; width:100%" bind="cfc:#request.cfc#cfc.bind.getBranches()" display="BranchID" value="BranchID" bindonload="yes"> </cfselect>
         </cfif>            
        </cfif>
    ...
    <tr>
      <td width="103"><div align="left">Account:</div></td>
      <td width="162"><div align="left">
        <cfif isdefined('AddInvoice')>
          <cfset AccountID = "#form.branchId#">
          ...
          <cfelse>
          <cfselect name="AccountID" id="AccountID" disabled  message="Select an Account"  tabindex="5" style="text-align:left; width:100%"> </cfselect>
        </cfif>
      </div></td>
      <td width="429">&nbsp;</td>
      ...
      <td><div align="left">
       ...
       where userid = '#GetAuthUser()#'
        ...
        <cfelse>
        <cfselect name="StaffID" id="StaffID" tooltip="Staff that signed the invoice" required="yes" message="Select Authorised" tabindex="6" style="text-align:left; width:100%" bind="cfc:#request.cfc#cfc.bind.getStaff({BranchID})" display="FirstName" value="FirstName" bindonload="yes">
        </cfselect>
        </cfif>
       </cfif>
      </div></td>
...
<button name="ok" id="ok">Ok</button>
    ...
    <tr>    
    <td><cfselect name="itemName" id="itemName"  style="width:100%" required="yes"  message="Select A Product" title="Item Name" ></cfselect></td>
    <td><cfinput name="Description" id="Description" type="text"  readonly="true" style='width:100%'  title="Description"></td>
    <td> <cfinput type="text" name="Quantity" id="Quantity" validate="integer" tabindex="9" required="yes" message="Enter quantity sold"  onKeyUp="checkValue();" style="text-align:right; width:100%" autosuggest="no"> </td>
    <td> <cfinput type="text" name="Price" id="Price" validate="float" tabindex="10" required="yes" message="Enter item price"  onKeyUp="checkValue();" style="text-align:right; width:100%" autosuggest="no"> </td>
...
<td align="right"><a href="#cgi.SCRIPT_NAME#?reset=1"><font color="##FF0000"><strong>Reset</strong></font></a><cfinput name="post" id="post" type="submit" value="Post" tabindex="15"></td>
</tr>
</cfform>
