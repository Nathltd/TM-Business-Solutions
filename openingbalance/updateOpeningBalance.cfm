...
<html>
<head>
<title><cfoutput>Open.Balance Modify :: #request.title#</cfoutput></title>
...
INSERT into OpeningBalance (OpNo,ProductID,Qty,CostPrice,BranchID,OpeningDate,LastUpdated,Creator) 
                VALUES ('#session.OpNo#','#Form.OBgrid.ProductID[counter]#','#Form.OBgrid.Qty[counter]#','#Form.OBgrid.CostPrice[counter]#','#session.BranchID#',
                <cfqueryparam value="#lsDateFormat(CreateODBCDate(request.StartYear), "dd/mm/yyyy")#" cfsqltype="cf_sql_date">, <cfqueryparam value="#lsDateFormat(CreateODBCDate(now()), "dd/mm/yyyy")#" cfsqltype="cf_sql_date">,'#GetAuthUser()#')
</cfquery>
...
OpeningDate = <cfqueryparam value="#lsDateFormat(CreateODBCDate(request.StartYear), "dd/mm/yyyy")#" cfsqltype="cf_sql_date">,
...
<tr>
<td>
<cfgrid name="OBgrid" width="380" height="300" query="OPdetails" insert="yes" insertbutton="Add"  delete="yes" deletebutton="Remove" gridlines="no" format="applet" fontsize="12"colheaderbold="Yes" colheaders="yes"
