...
<h4>Sales Search <cfoutput><cfif isdefined("form.Submit")> For #form.customerId#</cfif></cfoutput></h4>
<div id="RightColumn">
<cfparam name="url.print" default="">
...
<table width="90%">
  <tr>
    ...
    <td></td>
    <td valign="baseline"><div align="left"><font face="Segoe UI" size="-2">Customer:</font></div></td>
    <td valign="baseline"><div align="left"><strong><font face="Segoe UI" size="-2">#session.customerId#</font></strong></div></td>
  </tr>
  <tr>
...
SELECT SalesDate, SalesID, ProductID, BranchID, Qty, Amt, customerId
FROM vwSalesDetails
...
<tr align="center" valign="top">
  ...
  <td width="100"valign="top">
    <div align="left">
      <cfselect name="customerId" message="Choose Customer"  required="yes" id="customerId" style="width:100">
      <option value="All" selected>All</option>
      <cfloop query="Allcustomers">
      <option value="#customerId#" <cfif Allcustomers.customerId is #form.customerId#>
...
WHERE SalesDate between <cfqueryparam value="#dateformat(form.startDate,"dd/mm/yyyy")#" cfsqltype="cf_sql_date">
		...
		<cfelse>
		AND customerId = <cfqueryparam value="#trim(form.customerId)#"cfsqltype="cf_sql_clob">
        </cfif>
